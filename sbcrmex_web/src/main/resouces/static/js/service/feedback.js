...
// 加载表格数据
$JACK.initTable("service_list_feedback", {
    url: "service/pager",
    // 得到查询的参数
    ...
    columns: [
        ...
        },
        {
            ...
            title: "操作",
            formatter: function (value, row, index) {
                ret = '<button class="btn btn-sm btn-success" id="btn_service_feedback">处理</button>';
                return ret;
            },
            events: {
                "click #btn_service_feedback": function (e, value, row,
                                                         index) {
                    initFormData(row);
                    $("#servicefeedbackdialog").modal("show");
                }
            }
        }]
});
